// pages/env.js
// Edge runtime route that exposes limited, safe runtime config
// for the frontend (HayloAuth, Supabase client, Stripe public keys, etc).
//
// It writes window.__ENV__ and some globals when loaded in the browser.
...
export default async function handler(req) {
  ...
  function normalizeGetStartedUrl(raw, fallback) {
    ...
    // Only allow same-origin paths here to avoid weird redirects
    if (value.startsWith("/")) return value;
    return fallback;
  }

  const loginPath = canonicalLoginPath(
    process.env.NEXT_PUBLIC_HF_LOGIN_PATH || process.env.HF_LOGIN_PATH
  );
  ...
  const cfg = {
    ...
    // Optional: where tipped "thank you" pages live, for static share links
    THANK_HOST:
